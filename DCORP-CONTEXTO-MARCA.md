# DCORP Engenharia — Contexto de Marca

## Visão Geral

A DCORP Engenharia atua com planejamento, execução e entrega de resultados em projetos industriais.

A marca é posicionada como uma empresa que une inteligência construtiva, eficiência operacional e visão de crescimento, desenvolvendo soluções para construção industrial com foco em produtividade, racionalização e desempenho.

### Propósito

Construir hoje para gerar mais oportunidades amanhã.

### Assinatura

> Projetos que constroem oportunidades.

---

# Posicionamento

A DCORP deve ser percebida como:

- Técnica
- Confiável
- Inteligente
- Moderna
- Industrial
- Elegante
- Estratégica
- Orientada a resultados

A comunicação deve transmitir:

- Planejamento
- Execução
- Resultados
- Eficiência
- Tecnologia
- Sustentabilidade
- Crescimento

---

# Conceito da Marca

A identidade visual é construída sobre três pilares:

## Planejamento

Representa:

- Estratégia
- Inteligência
- Viabilidade

## Pessoas

Representa:

- Confiança
- Parceria
- Relacionamentos

## Execução

Representa:

- Resultados
- Solidez
- Crescimento

---

# Segmento

## Especialidade

Construção Industrial

A empresa adota sistemas construtivos industrializados para:

- Ganho de produtividade
- Redução de desperdícios
- Melhoria da qualidade
- Redução de prazos
- Maior previsibilidade de obra

---

# Sistemas Construtivos

Regra de linguagem: preferir **"sistemas construtivos industrializados e soluções de alta produtividade"**. Não apresentar o portfólio como “construção a seco” — EPS, ICF e paredes de concreto utilizam argamassa ou concreto. Nas páginas, classificar cada sistema como seco, semi-industrializado ou moldado in loco.

## Painel Monolítico EPS

Tipo: semi-industrializado (com argamassa/concreto)

Benefícios:

- Leveza
- Desempenho térmico
- Rapidez de montagem

## Paredes de concreto moldadas in loco

Tipo: moldado in loco

Benefícios:

- Robustez estrutural
- Controle construtivo
- Padronização de qualidade

## ICF — formas isolantes para concreto

Tipo: semi-industrializado (forma + concreto)

Benefícios:

- Isolamento térmico
- Velocidade de execução
- Desempenho energético

## Lightwall

Tipo: seco / leve

Benefícios:

- Agilidade
- Flexibilidade de layout
- Produtividade em obra

Outros sistemas industrializados só entram no site após confirmação de domínio efetivo pela empresa.

---

# Proposta de Valor

A DCORP entrega:

- Planejamento estruturado
- Execução eficiente
- Controle de qualidade
- Soluções industrializadas
- Maior produtividade
- Redução de desperdícios
- Crescimento sustentável
- Resultados mensuráveis

---

# Tom de Voz

## Deve ser

- Profissional
- Seguro
- Técnico
- Objetivo
- Sofisticado
- Confiante
- Direto

## Evitar

- Linguagem informal
- Exageros de marketing
- Promessas vazias
- Jargões excessivos
- Comunicação agressiva

---

# Mensagens-Chave

## Institucional

"Projetos que constroem oportunidades."

## Posicionamento

"Planejamento, execução e resultados."

## Construção Industrial

"Soluções industrializadas para ganhar produtividade, reduzir desperdícios e acelerar prazos."

## Valor

"Eficiência, tecnologia e crescimento para projetos industriais."

---

# Arquétipo da Marca

## Primário

Construtor

Características:

- Cria estruturas duradouras
- Busca eficiência
- Gera desenvolvimento
- Entrega resultados concretos

## Secundário

Sábio

Características:

- Planejamento
- Inteligência técnica
- Estratégia
- Conhecimento aplicado

---

# Direção Visual

## Estilo

Minimalista industrial premium.

Mistura:

- Engenharia
- Arquitetura corporativa
- Construção industrial
- Sofisticação empresarial

## Elementos Visuais

- Linhas finas douradas
- Grandes áreas em branco
- Geometria limpa
- Fotografia arquitetônica industrial
- Estruturas de concreto aparente
- Composição assimétrica
- Espaços amplos
- Contraste entre claro e escuro

## Sensações Desejadas

- Solidez
- Organização
- Precisão
- Modernidade
- Escala
- Credibilidade

---

# Paleta de Cores

## Grafite

HEX: #1F1F1F

Significados:

- Solidez
- Confiança

## Dourado

HEX: #C9A96A

Significados:

- Valor
- Crescimento

## Cinza Escuro

HEX: #4D4D4D

Significados:

- Equilíbrio
- Profissionalismo

## Cinza Claro

HEX: #D9D9D9

Significados:

- Clareza
- Estrutura

## Branco

HEX: #FFFFFF

Significados:

- Transparência
- Leveza

---

# Tipografia

## Títulos

Montserrat Bold

Características:

- Forte
- Moderna
- Impactante

## Subtítulos

Montserrat SemiBold

Características:

- Hierarquia clara
- Boa leitura

## Texto Corrido

Montserrat Regular

Características:

- Limpeza
- Legibilidade
- Consistência

---

# Diretrizes para Landing Pages

## Hero Section

Objetivo:

Mostrar autoridade e posicionamento imediatamente.

Elementos recomendados:

- Headline forte
- Imagem de obra industrial
- Destaque para planejamento, execução e resultados
- CTA claro

Exemplo:

"Construção industrial com planejamento, execução e resultados."

---

## Sessão de Diferenciais

Destacar:

- Inteligência construtiva
- Eficiência operacional
- Controle de qualidade
- Redução de desperdícios
- Entrega previsível
- Crescimento sustentável

---

## Sessão de Soluções

Apresentar:

- Painel Monolítico EPS
- Paredes de concreto moldadas in loco
- ICF — formas isolantes para concreto
- Lightwall

---

## Sessão Institucional

Explicar:

- Quem é a DCORP
- Como trabalha
- Filosofia de planejamento e execução

---

## Sessão de Credibilidade

Inserir:

- Portfólio
- Obras
- Cases
- Parceiros
- Números relevantes

---

## CTA Final

Mensagem recomendada:

"Vamos construir oportunidades para o seu projeto."

Botão:

"Solicitar orçamento"

ou

"Falar com um especialista"

---

# Resumo Estratégico

A DCORP Engenharia é uma empresa de construção industrial posicionada como parceira estratégica para projetos que exigem eficiência, inteligência construtiva e resultados concretos.

Sua comunicação deve equilibrar:

- Sofisticação
- Engenharia
- Crescimento
- Confiabilidade

Sempre transmitindo a ideia central:

"Planejamento. Execução. Resultados."

---

# IA do site (brief comercial)

## Posicionamento comercial

Empresa de engenharia e construção especializada em sistemas construtivos industrializados, com atuação própria e prestação de serviços para incorporadoras, investidores, empresas e clientes terceiros.

O site deve transmitir especialização técnica, produtividade, engenharia e capacidade de execução.

## Abertura (hero)

Assinatura de marca (headline):

> Projetos que constroem oportunidades.

Supporting line (brief):

> Construção industrializada com engenharia, velocidade e controle — para incorporadoras, investidores e empresas.

## Serviços

- Planejamento executivo de obras
- Orçamentação e engenharia de custos
- Compatibilização de projetos
- Construção de unidades habitacionais
- Execução de condomínios e empreendimentos residenciais
- Implantação de sistemas construtivos industrializados
- Gerenciamento e fiscalização de obras
- Treinamento e apoio técnico de equipes
- Execução para incorporadoras e investidores
- Obras próprias e para terceiros

## Públicos

- Incorporadoras
- Construtoras que necessitam de parceiro tecnológico
- Investidores imobiliários
- Proprietários de terrenos
- Empresas interessadas em construção rápida
- Clientes institucionais
- SPEs, inclusive externas ao grupo

## Menu canônico

- Início
- A DCORP (`/quem-somos`)
- Sistemas construtivos
- Serviços
- Obras e portfólio
- Contato

CTA do header: **Solicitar orçamento**

“Seja nosso parceiro” não é item de menu — bloco em Contato ou Serviços.
