    ...
    <section className="py-32 px-6 md:px-24 bg-surface overflow-hidden">
      <div className="text-center mb-24">
        <motion.span 
          ...
          viewport={{ once: true }}
          className="text-primary font-label text-xs uppercase tracking-[0.4em] block mb-4"
        >
          Governança
        </motion.span>
        <motion.h2 
        ...
        {partners.map((partner, index) => {
          ...
          return (
            <motion.div 
              ...
              className="bg-surface-container-high border border-outline-variant/10 group overflow-hidden hover:border-primary/30 transition-all duration-500 flex flex-col self-start w-full"
            >
              ...
              <div className="p-8 flex-grow flex flex-col">
                <h3 className="font-headline text-xl text-primary mb-1 group-hover:text-primary transition-colors">{partner.name}</h3>
                <p className="text-[10px] font-label uppercase tracking-widest text-on-surface-variant mb-6 min-h-[40px] leading-relaxed">
                  {partner.role}
                </p>
                <div className="w-8 h-[1px] bg-primary/40 mb-6 group-hover:w-12 transition-all duration-500"></div>
                
                <div className="relative">
                  <p className={`font-headline text-sm text-on-surface/80 leading-relaxed italic transition-all duration-300 ${isExpanded ? '' : 'line-clamp-4'}`}>
                    {partner.description}
                  </p>
                  <AnimatePresence>
                    {isExpanded && (
                      ...
                      >
                        <p className="font-headline text-sm text-on-surface/80 leading-relaxed italic mt-4">
                          {partner.extraInfo}
                        </p>
                      </motion.div>
                    )}
                  ...
                  <button 
                    ...
                    className="mt-4 text-[10px] text-primary uppercase tracking-widest font-bold hover:opacity-70 transition-opacity cursor-pointer flex items-center gap-1"
                  >
                ...
                <div className="mt-8 pt-6 border-t border-outline-variant/10">
                  <span className="text-[10px] font-label uppercase tracking-widest text-primary block mb-1">Unidade</span>
                  <span className="text-xs text-on-surface-variant font-medium">{partner.company}</span>
                </div>
              </div>
            </motion.div>
          );
